...
pub fn find_interactive(
    ...
    let all = read_contacts(&dir_contacts);
        ...
        match vcards.len().cmp(&1) {
            Ordering::Less    => {println!("Aucun résultat pour cette recherche.");}
...
pub fn find_vcards(all: String, property_find: &PropertyType, value_find: &String, property_show: &PropertyType) -> Vec<Vcard> {
    let vcards = parse_to_vcards_without_errors(&all);
    let mut vcards_found: Vec<Vcard> = Vec::new();
    for vcard in vcards {
        if !is_property_present(&vcard, property_show) {continue}
	...
	for property in properties {
		...
		for x in patterns {
		if value_present.contains(x) { vcards_found.push(vcard);}
...
fn is_property_present(vcard: &Vcard, property_show: &PropertyType) -> bool {
    if vcard.get_properties_by_type(property_show).is_empty() {false}
    else {true}
